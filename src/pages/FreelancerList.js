...
const FreelancerList = () => {
  ...
  const handleSaveNewFreelancer = () => {
    ...
    if (!newFreelancer.Username || !newFreelancer.Email || !newFreelancer.PhoneNumber || !newFreelancer.Skillsets || !newFreelancer.Hobby) {
    ...
    <div>
          ...
          {currentFreelancers.length === 0 ? (
            ...
            currentFreelancers.map((freelancer, index) => (
              <tr key={freelancer.Id}>
                <td>{indexOfFirstItem + index + 1}</td> {/* Use the correct index */}
                <td>{freelancer.Id}</td>
          ...
          <tr>
            <td colSpan="9">
              ...
              <div className="pagination btn_paging">
                {Array.from({ length: Math.ceil(filteredFreelancers.length / itemsPerPage) }).map((_, index) => (
                  <Button key={index} variant="outline-primary" onClick={() => handlePageChange(index + 1)}>
                    {index + 1}
                  </Button>
                ))}
                &nbsp; &nbsp; 
                <span className="page-label">Page {currentPage} of {Math.ceil(filteredFreelancers.length / itemsPerPage)}</span>
              </div>
            </td>
          </tr>
        </tfoot>
        ...
        <Modal.Body>
          <Form>
            <Form.Group controlId="username">
              ...
              <Form.Control type="text" name="Username" value={newFreelancer.Username} onChange={handleInputChange} />
              {showAlert && !newFreelancer.Username && <Alert variant="danger">Username is required!</Alert>}
            </Form.Group>
            <Form.Group controlId="email">
              <Form.Label>Email</Form.Label>
              <Form.Control type="email" name="Email" value={newFreelancer.Email} onChange={handleInputChange} />
              {showAlert && !newFreelancer.Email && <Alert variant="danger">Email is required!</Alert>}
            </Form.Group>
            <Form.Group controlId="phone">
              <Form.Label>Phone Number</Form.Label>
              <Form.Control type="text" name="PhoneNumber" value={newFreelancer.PhoneNumber} onChange={handleInputChange} />
              {showAlert && !newFreelancer.PhoneNumber && <Alert variant="danger">Phone Number is required!</Alert>}
            </Form.Group>
            <Form.Group controlId="skillsets">
              <Form.Label>Skillsets</Form.Label>
              <Form.Control type="text" name="Skillsets" value={newFreelancer.Skillsets} onChange={handleInputChange} />
              {showAlert && !newFreelancer.Skillsets && <Alert variant="danger">Skillsets is required!</Alert>}
            </Form.Group>
            <Form.Group controlId="hobby">
              <Form.Label>Hobby</Form.Label>
              <Form.Control type="text" name="Hobby" value={newFreelancer.Hobby} onChange={handleInputChange} />
              {showAlert && !newFreelancer.Hobby && <Alert variant="danger">Hobby is required!</Alert>}
            </Form.Group>
          </Form>
        </Modal.Body>
        <Modal.Footer>
      ...
      <Modal show={showDeleteModal} onHide={handleCancelDelete}>
        ...
        <Modal.Footer>
          ...
          <Button variant="danger" onClick={handleContinueDelete}>Continue</Button>
        </Modal.Footer>
      </Modal>
    </div>
  );
